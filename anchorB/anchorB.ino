...
void setup() {
  ...
  DW1000.setAntennaDelay(16384); //Default 16384
//  DW1000.setAntennaDelay(settings.antenna);
  ...
  DW1000Ranging.startAsAnchor("0B:00:5B:D5:A9:9A:E2:9C", DW1000.MODE_LONGDATA_FAST_LOWPOWER, DW1000.CHANNEL_5, false);
  ...
  if (namezzz == settings.tag) {
    ...
    if (current_distance <= settings.range && current_distance > 0) {
      ...
      if (IS_DEBUG) {
        ...
        Serial.print(DW1000Ranging.getDistantDevice()->getRange()); Serial.print(F(" m"));
        Serial.print(F("\t Avg Range: ")); 
        Serial.print(avg_distance); Serial.println(" m");
      }
